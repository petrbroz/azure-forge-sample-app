...
module.exports = async function (context, req) {
    context.log('JavaScript HTTP trigger function processed a model list request.');
    if (!FORGE_CLIENT_ID || !FORGE_CLIENT_SECRET) {
        const msg = 'FORGE_CLIENT_ID or FORGE_CLIENT_SECRET env. variables are missing.';
        console.error(msg);
        context.res = { status: 400, body: msg };
    } else if (!req.params.bucket) {
        const msg = '{bucket} parameter missing in the URL.';
        console.error(msg);
        context.res = { status: 400, body: msg };
    } else {
        try {
            const client = new DataManagementClient();
            const objects = await client.listObjects(req.params.bucket);
            context.res = {
